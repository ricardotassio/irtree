...
public class RawAccessUtils {
	 ...
	 */
	public static void fillRawAccess(RawAccess ra, int value) {
		byte[] block = new byte[ra.getSectorSize()];
		java.util.Arrays.fill(block, (byte) value);
		for (long i=0; i<ra.getNumSectors() ; i++) {
			ra.write(block, i);
		}
	}
	 ...
	 */
	public static boolean checkRawAccess(RawAccess ra, byte b) {
		byte[] block = new byte[512];
		long numberOfSectors = ra.getNumSectors();
	...
	/**
	 * Returns printable information of a raw access as a String.
	 * 
	 * @param ra the given raw access
	 * @return String with Information
	 */
	public static String rawAccessInfo(RawAccess ra) {
		return ra.getClass().getName()+", sectors: "+ra.getNumSectors()+", sectorSize: "+ra.getSectorSize();
	}
	...
	public static void main(String args[]) {
		...
		createFileForRaw(args[0], Long.parseLong(args[1]));
	}
}
