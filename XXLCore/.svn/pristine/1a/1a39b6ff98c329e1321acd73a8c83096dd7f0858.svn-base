...
/**
 * This class has all operations that has RandomAccessFile, but uses
 * a raw access. The maximum file size is the length of the 
 * raw access (in bytes) minus 8 bytes.
...
public class RawAccessRAF extends RandomAccessFile {
	 ...
	 */
	public RawAccessRAF(RawAccess ra, boolean create, File dummyFile) throws FileNotFoundException {
		...
		if (!create) {
			...
			// System.out.println("SimpleFilesystemRAF Constructor: length="+fileLength);
		}
		else {
	...
	private void readSector() {
		if (currentSectorInBuffer!=filePointerSector) {
			...
			if (filePointer>fileLength) {
				...
				long numberOfSectors = ((fileLength+8+bytesPerSec-1)/bytesPerSec); // size in sectors so far
	...
	public void close() throws IOException {
		...
		if (oldFileLength != fileLength) {
			// System.out.println("SimpleFilesystemRAF Close: length="+fileLength);
