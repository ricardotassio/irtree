...
public class BinarySearchTree {
	...
	public Iterator rangeQuery(final Comparator chooseSubtree, final Object [] minkey, final Object [] maxkey, final boolean forwards) {
		...
		// let forwards be true ==> (if result[0]!=0 ==> result[0]>0)
		if (result[0]!=0) // object not found
			// alternatively: if ((forwards?1:-1)*chooseSubtree.compare(forwards?minkey:maxkey,n)>0)
			n = n.next(forwards?1:0);
	...
	public static void main(String [] args) throws Exception {
		Predicate aggregateFunction = new Predicate () {
			public boolean invoke (Object argument) {
				...
				if (node.children[0]!=null)
					in.agg_right = Math.max(in.agg_right,((Interval) node.children[0].object()).agg_right);
				if (node.children[1]!=null)
					in.agg_right = Math.max(in.agg_right,((Interval) node.children[1].object()).agg_right);
		...
		BinarySearchTree tree = new BinarySearchTree(aggregateFunction,Function.IDENTITY);

		final Comparator comparator = ComparableComparator.DEFAULT_INSTANCE;
		...
		tree.insert(chooseSubtree,new Object[] {tree.new Interval(5,7)});
		tree.insert(chooseSubtree,new Object[] {tree.new Interval(3,4)});
		tree.insert(chooseSubtree,new Object[] {tree.new Interval(7,8)});
		tree.insert(chooseSubtree,new Object[] {tree.new Interval(9,11)});
		tree.insert(chooseSubtree,new Object[] {tree.new Interval(6,12)});

		Iterator it;

		System.out.println("Perform a range query forwards.");
		it = tree.rangeQuery(chooseSubtree,new Object [] {tree.new Interval(3,0)},new Object [] {tree.new Interval(7,0)},true);
  		while (it.hasNext()) {
		...
		it = tree.rangeQuery(chooseSubtree,new Object [] {tree.new Interval(3,0)},new Object [] {tree.new Interval(7,0)},false);
  		while (it.hasNext()) {
  			Interval in = (Interval) ((Node) it.next()).object();
			System.out.println("["+in.left+","+in.right+"] agg="+in.agg_right);
		}
