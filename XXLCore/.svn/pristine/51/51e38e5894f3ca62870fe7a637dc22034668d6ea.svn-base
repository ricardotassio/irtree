...
public class PartitionRawAccess implements RawAccess {
	...
	public void write(byte[] block, long sector) {
		if ((sector<0) || (sector>=size))
			throw new RawAccessError("Attempt to write outside boundary ("+sector+", length:"+size+", startSector="+startSector+")");
	...
	public void read(byte[] block, long sector) throws RawAccessError {
		if ((sector<0) || (sector>=size))
			throw new RawAccessError("Attempt to read outside boundary ("+sector+", length:"+size+", startSector="+startSector+")");
