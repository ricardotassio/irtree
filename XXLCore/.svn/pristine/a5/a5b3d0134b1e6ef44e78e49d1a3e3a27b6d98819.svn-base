 ...
 * <tt>Integer</tt> objects. In addition to the read and write methods
 * that read or write <tt>Integer</tt> objects this class contains readInt
 * and writeInt methods that convert the <tt>Integer</tt> object after
 * reading or before writing it to its primitive <tt>int</tt> type.<p>
 *
 * Example usage (1).
 * <pre>
 *     // catch IOExceptions
 ...
 *
 *         IntegerConverter.DEFAULT_INSTANCE.write(new DataOutputStream(output), new Integer(42));
 ...
 *         Integer i2 = (Integer)IntegerConverter.DEFAULT_INSTANCE.read(new DataInputStream(input));
 *
 ...
 * @see IOException
 */
public class IntegerConverter extends FixedSizeConverter {

	/**
	 * This instance can be used for getting a default instance of
	 * IntegerConverter. It is similar to the <i>Singleton Design
	 ...
	 * creation of other instances of IntegerConverter.
	 */
	 ...
	 * @param object the (<tt>Integer</tt>) object to be restored. In this
	...
	/**
	 * Reads the <tt>int</tt> value from the specified data input and
	 * returns it. <br>
	 ...
	 * This implementation calls the writeInt method of the data output
	 * with the <tt>int</tt> value of the object.
	 *
	...
	/**
	 * Writes the specified <tt>int</tt> value to the specified data
	 * output. <br>
	 * This implementation calls the write method with an
	 * <tt>Integer</tt> object wrapping the specified <tt>int</tt> value.
	 *
	 * @param dataOutput the stream to write the specified <tt>int</tt>
	 *        value to.
	 ...
	 * IntegerConverter. It can also be used to test the functionality of
	 * an IntegerConverter.
	 *
	...
	public static void main (String [] args) {
		...
		try {
			...
			// write an Integer and an int value to the output stream
			IntegerConverter.DEFAULT_INSTANCE.write(new java.io.DataOutputStream(output), new Integer(42));
			...
			Integer i2 = (Integer)IntegerConverter.DEFAULT_INSTANCE.read(new java.io.DataInputStream(input));
			// print the value and the object
			System.out.println(i1);
			System.out.println(i2);
			// close the streams after use
