...
public class HybridKernelCDF extends AbstractKernelCDF {
	...
	public static Function FACTORY = new Function() {
		public Object invoke(Object[] o) {
			...
			//
			return new HybridKernelCDF((KernelFunction) o[0],
			// kernel function
			 (Object[]) o[1], // sample
			h, // type of the used bandwidths
			 ((Number) o[3]).doubleValue(), // minimum
			 ((Number) o[4]).doubleValue(), // maximum
			 (Object[]) o[5] // jump points/change points/bin borders
			);
		}
	};
	...
	protected void reinit() {
		...
		for (int i = 0; i < binEstimators.length; i++) {
			parameters = new Object[] { kf, binSamples[i], new Double(hs[i]), borders[i], borders[i + 1] };
