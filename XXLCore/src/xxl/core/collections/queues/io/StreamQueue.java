...
public abstract class StreamQueue extends AbstractQueue {
	 ...
	 */
	public StreamQueue(Converter converter, Function inputBufferSize, Function outputBufferSize, Function newInputStream, Function newOutputStream) {
		super();
	 ...
	 *        writing the serialized elements of this queue.
	 */
	public StreamQueue(Converter converter, Function newObject, Function inputBufferSize, Function outputBufferSize, Function newInputStream, Function newOutputStream) {
		this(new UniformConverter(converter, newObject), inputBufferSize, outputBufferSize, newInputStream, newOutputStream);
	}

	/**
	 * Constructs a new stream queue that uses a convertable converter for
	 * serializing and de-serializing its elements. The factory method is
	 * used for initializing the elements of the queue before the output
	 * stream is read out. The given functions are used for determining
	 * the size of the input and output buffer and opening input and output
	 * streams.<br>
	 * This constructor is equivalent to the call of
	 * <code>StreamQueue(ConvertableConverter.DEFAULT_INSTANCE, newObject, inputBufferSize, outputBufferSize)</code>.
	 *
	 * @param newObject a factory method that is used for initializing the
	 *        elements of the queue before the input stream is read out.
	 * @param inputBufferSize a function that determines the size of the
	 ...
	 */
	public StreamQueue(Function newObject, Function inputBufferSize, Function outputBufferSize, Function newInputStream, Function newOutputStream){
		this(ConvertableConverter.DEFAULT_INSTANCE, newObject, inputBufferSize, outputBufferSize, newInputStream, newOutputStream);
	}
	...
	protected void openInput() {
		...
		if (dataInputStream == null) {
			int bufferSize = ((Integer)inputBufferSize.invoke()).intValue();
			dataInputStream = new DataInputStream(
				bufferSize > 0 ?
					new BufferedInputStream((InputStream)newInputStream.invoke(), bufferSize) :
					(InputStream)newInputStream.invoke()
			);
		}
	}
	...
	protected void openOutput() {
		...
		if (dataOutputStream == null) {
			int bufferSize = ((Integer)outputBufferSize.invoke()).intValue();
			dataOutputStream = new DataOutputStream(
				bufferSize > 0 ?
					new BufferedOutputStream((OutputStream)newOutputStream.invoke(), bufferSize) :
					(OutputStream)newOutputStream.invoke()
			);
		}
	}
