...
public class AggregateGrouper extends AbstractCursor {
	 ...
	 * {@link xxl.core.functions.Function#IDENTITY identity} function for the
	...
	private void consume(Object next) {
		...
		while (it.hasNext() && notFound) {
			...
			if (equals.invoke(value, rep[0])) {
				notFound = false;
				rep[1] = ((Function)rep[2]).invoke(rep[1], next);
			}
		}
		if (notFound) {
			...
			c[0] = value;
			c[1] = ((Function)c[2]).invoke(null, next);
			cumulative.add(c);
		}
	}
	...
	public static void main(String[] args) {
		...
		xxl.core.cursors.Cursor cfd = new CFDCursor(
		...
		xxl.core.cursors.Cursor histogram = new AggregateGrouper(
			new xxl.core.cursors.sources.RandomIntegers(10, 500),
			new Function() {
				int start = 0;
				int binWidth = 3;
				public Object invoke(Object integer) {
			...
			new Function() {
				public Object invoke() {
					return new xxl.core.math.statistics.parametric.aggregates.Average();
				}
			}
		);
