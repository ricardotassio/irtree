 ...
 * <tt>Character</tt> objects as <tt>ASCII</tt> characters (the
 * <tt>byte</tt> values representing the <tt>ASCII</tt> character). In
 * addition to the read and write methods that read or write
 * <tt>Character</tt> objects this class contains readChar and writeChar
 * methods that convert the <tt>Character</tt> object after reading or
 * before writing it to its primitive <tt>char</tt> type.<p>
 *
 * Example usage (1).
 * <pre>
 *     // catch IOExceptions
...
public class AsciiConverter extends CharacterConverter {
	 ...
	 * AsciiConverter. It is similar to the <i>Singleton Design
	 ...
	 * creation of other instances of AsciiConverter.
	 */
	 ...
	 * <tt>byte</tt> value representing the <tt>ASCII</tt> character of a
	 * <tt>Character</tt> object. Because this size is predefined it must
	...
	/**
	 * Reads the <tt>byte</tt> value representing the <tt>ASCII</tt>
	 * character for the specified (<tt>Character</tt>) object from the
	 ...
	 *
	 * @param dataInput the stream to read the <tt>byte</tt> value
	 *        representing the <tt>ASCII</tt> character from in order to
	 *        return a <tt>Character</tt> object.
	...
	public Object read (DataInput dataInput, Object object) throws IOException {
		return new Character((char)dataInput.readByte());
	}

	/**
	 * Writes the <tt>byte</tt> value representing the <tt>ASCII</tt>
	 * character of the specified <tt>Character</tt> object to the
	 * specified data output. <br>
	 * This implementation calls the write method of the data output with
	 * the <tt>byte</tt> value representing the <tt>ASCII</tt> character
	 * of the object.
	 *
	 * @param dataOutput the stream to write the <tt>byte</tt> value
	 *        representing the <tt>ASCII</tt> character of the specified
	 *        <tt>Character</tt> object to.
	 * @param object the <tt>Character</tt> object that <tt>byte</tt>
	 *        value representing the <tt>ASCII</tt> character should be
	 *        written to the data output.
	 * @throws IOException includes any I/O exceptions that may occur.
	 */
	public void write (DataOutput dataOutput, Object object) throws IOException {
		dataOutput.write(((Character)object).charValue());
	}

	/**
	 * The main method contains some examples how to use an
	 * AsciiConverter. It can also be used to test the functionality of an
	 * AsciiConverter.
	 *
	...
	public static void main (String [] args) {
		...
		try {
			...
			// write a Character and a char value to the output stream
			AsciiConverter.DEFAULT_INSTANCE.write(new java.io.DataOutputStream(output), new Character('C'));
			...
			Character c2 = (Character)AsciiConverter.DEFAULT_INSTANCE.read(new java.io.DataInputStream(input));
			// print the value and the object
			System.out.println(c1);
			System.out.println(c2);
			// close the streams after use
