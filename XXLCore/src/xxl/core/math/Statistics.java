...
public class Statistics {
	...
	*/
	...
	public static Object[] doubleArrayCFD(Iterator cfd) {
		return doubleArrayCFD(cfd, new Function() {
	...
	public static double gaussian(double x, double mean, double variance)
		...
		double sigma = Math.pow(variance, 0.5);

		double r = 0.0;
		r = -1.0 * (x - mean) * (x - mean);
		r = r / (2.0 * variance);
		r = Math.exp(r);
		r = r / (Math.pow(2.0 * Math.PI * sigma, 0.5));
		return r;
	}
	...
	public static double triweightPrimitive(double x, double c) {
		...
		if ((x >= -1) && (x <= 1)) {
			r += 1.0
				+ 0.6 * (x * x * x * x)
				- (x * x)
				+ (x * x * x * x * x * x) * 0.14285714285714285714285714285714;
			r *= 1.09375 * x;
			r += c;
		}
		return r;
	...
	public static void main(String[] args) {
		...
		for (int i = 1; i <= steps; i++) {
			...
			System.out.println(p + "-quantil of N(0,1)\t=" + normalQuantil(p));
		}
		System.out.println();
		...
		for (int i = 0; i <= steps; i++) {
			...
			System.out.println(
				p
					...
					+ triweight(p)
					+ "\t"
					+ triweightDerivative(p)
					+ "\t"
					+ triweightPrimitive(p));
		}
		System.out.println();
		...
		for (int i = 0; i <= steps; i++) {
			...
			System.out.println(
				p
					...
					+ cosineArch(p)
					+ "\t"
					+ cosineArchDerivative(p)
					+ "\t"
					+ cosineArchPrimitive(p));
		}
		System.out.println();
		...
		Cursor cfd = new CFDCursor(new RandomIntegers(10, 100));
		Object[] cfdComp = doubleArrayCFD(cfd);
		System.out.println("\nvalues:");
		System.out.println(Strings.toString((double[]) cfdComp[0]));
		System.out.println("frequencies:");
		System.out.println(Strings.toString((int[]) cfdComp[1]));
		...
		System.out.println(Strings.toString(grid));
		System.out.println(Strings.toString(evalReal1DFunction ( grid, gaussian)) );
		
		Function spline = new RB1CubicBezierSpline(grid, evalReal1DFunction(grid, gaussian));
		for (int i = -30; i < 31; i++) {
			double x = i / 10.0;
			System.out.println(
				"gaussian ("
