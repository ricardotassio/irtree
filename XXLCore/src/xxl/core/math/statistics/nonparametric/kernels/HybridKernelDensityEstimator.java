...
public class HybridKernelDensityEstimator extends AbstractKernelDensityEstimator {
	...
	public static Function FACTORY = new Function() {
		public Object invoke(Object[] o) {
				return new HybridKernelDensityEstimator((KernelFunction) o[0], // kernel function
				 (Object[]) o[1], // sample
				 (Object[]) o[2], // change points
				 (double[]) o[3], // bandwidths
				 ((Number) o[4]).doubleValue(), // min
				 ((Number) o[5]).doubleValue(), // max
				 ((Number) o[6]).doubleValue() // alpha
				);
		}
	};
	...
	protected void reinit() {
		...
		for (int i = 0; i < binEstimators.length; i++) {
			parameters =
				new Object[] { kf, binSamples[i], new Double(hs[i]), borders[i], borders[i + 1], new Double(alpha)};
	...
	public static void main(String[] args) {
		...
		System.out.println(xxl.core.util.Strings.toString(data, ", "));
		Object[] borders = new Object[] { new Double(-10), new Double(2), new Double(0.0), new Double(10)};
		System.out.println("borders:");
		System.out.println(xxl.core.util.Strings.toString(borders, ", "));
		System.out.println("-------------------------");
		Object[] splits = split(data, borders);
		for (int i = 0; i < splits.length; i++)
			System.out.println("split " + i + ":" + xxl.core.util.Strings.toString((Object[]) splits[i], ", "));
