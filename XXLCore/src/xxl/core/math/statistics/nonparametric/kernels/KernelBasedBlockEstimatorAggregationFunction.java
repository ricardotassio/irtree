...
public class KernelBasedBlockEstimatorAggregationFunction extends Function {
	...
	public static Cursor inputCursor(Iterator input, int blockSize) {
		return new Aggregator(
			input,
			new Function[] { new LastN(blockSize), new VarianceEstimator(), new Minimum(), new Maximum()});
	}
	...
	public Object invoke(Object old,Object next) { // next[0] = sample, next[1] = Double-Object with variance, next[2] = min , next[3] = max
		...
		if (build) {
			double var = ((Number) aggregate[1]).doubleValue();
			double min = ((Number) aggregate[2]).doubleValue();
			double max = ((Number) aggregate[3]).doubleValue();
			...
			Object[] parameters = new Object[aggregate.length + 1];
			parameters[0] = kf;
			parameters[1] = sample;
			parameters[2] = new Double(h);
			parameters[3] = new Double(min);
			parameters[4] = new Double(max);
			// Further computed aggregates just forwarding
			for (int i = 4; i < aggregate.length; i++)
				parameters[i + 1] = aggregate[i];
