 ...
 */
 public class BitSet implements Cloneable, Comparable, Convertable {
	 ...
	 * This number can be choosen smaller than the size of a bit set,
	...
	public BitSet(BitSet b){
		this((long[])b.bits.clone(), b.precision);
	}
...
//		bits[bitIndex>>>6] &= ~(1l<<(bitIndex%64));
	...
	*/
	...
	public int compare2(BitSet b, int noLongs){
		for(int i=0 ; i < noLongs; i++){
			...
			if((bits[i] & first) != (b.bits[i] & first)){
				if(bits[i] < b.bits[i]) //ACHTUNG: kehrt sich hier um!
					return +1;
				else
					return -1;
			}
	 ...
	 * otherwise 0 is returned.
	 *
	 * @param o the BitSet to be compared with the BitSet
	 * 		calling this method.
	 * @return a negative value if this (the calling) BitSet is smaller than
	 * 		the given BitSet o, a positive value if BitSet o is smaller,
	 * 		else 0 is returned.
